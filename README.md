...
**winget とは？** — Windows に標準で入っているアプリ管理コマンドです（Windows 10 / 11 なら最初から使えます）。
「コマンドで使えるアプリストア」のようなもので、Microsoft が運営するカタログからアプリを検索・インストール・更新・削除できます。

ブラウザで公式サイトを探してインストーラーをダウンロードする代わりに、**1行打てば同じことが終わる**のが利点。
打ったコマンドはメモに残せるので、PC の買い替えや再セットアップのとき「同じ1行を打つだけ」で環境を再現できます。

名前しか知らないアプリを入れるときの実務手順はこの3段階:
